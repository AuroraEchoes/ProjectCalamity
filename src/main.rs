...
fn main() {
    let sector =
        terrain_old::generate_terrain(Vector2::new(16, 16), "New terrain test sector".to_string());
...
fn render(renderer: &mut Renderer, game_data: &GameData, punyworld: &TextureAtlasHandle, overlays: &TextureAtlasHandle) {
    ...
    if let Some(sel) = game_data.selected_tile() {
        renderer.submit_textured_quad(TexturedQuad::new(Vector2::new((sel.x * edge_len) as i32, (sel.y * edge_len) as i32), Vector2::new(edge_len as i32, edge_len as i32), overlays.texture(0, 0)));
